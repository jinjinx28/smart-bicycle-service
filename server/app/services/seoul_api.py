...
def load_csv_data_once():
    ...
    if not df.empty:
        ...
        df = df[
            df["대여소명"].str.contains(
                "강남|역삼|논현|삼성|선릉|도곡|압구정|신사|대치|개포", na=False
            )
        ]
        ...
        if "대여소번호" in df.columns and "대여소명" in df.columns:
            ...
            try:
                grouped = (
                    df.groupby(["대여소번호", "대여소명"], as_index=False).agg(
                        agg_dict
                    )
                    if agg_dict
                    else df
        ...
        for idx, row in grouped.iterrows():
            ...
            status = (
                "GOOD"
                if available >= 5
                else ("LOW" if available > 0 else "EMPTY")
            )
...
def fetch_stations(
    ...
    for station in stations:
        ...
        dist = calculate_distance(
            target_lat, target_lng, s_copy["lat"], s_copy["lng"]
        )
...
def fetch_bike_routes(
    region: str = None, bike_type: str = None, difficulty: str = None
):
    ...
    if region and region != "전체":
        filtered_routes = [
            r for r in filtered_routes if r["region"] == region
        ]
    if bike_type and bike_type != "전체":
        filtered_routes = [
            r for r in filtered_routes if r["bikeType"] == bike_type
        ]
    if difficulty and difficulty != "전체":
        filtered_routes = [
            r for r in filtered_routes if r["difficulty"] == difficulty
        ]
...
def fetch_hourly_usage() -> list[dict]:
    stations = fetch_stations()
    base_count = (
        sum(s.get("available", 0) for s in stations) if stations else 50
    )
    hourly_data = []
    for h in range(24):
        hourly_data.append(
            {"hour": f"{h:02d}:00", "count": max(5, (base_count + h) % 40)}
        )
    return hourly_data