...
export default function AIAnalysis() {
  ...
  const monthlyUsage = Array.isArray(data.monthlyUsage) ? data.monthlyUsage : [];
  const topStations = Array.isArray(data.topStations) ? data.topStations : [];
  const ageDistribution = Array.isArray(data.ageDistribution) ? data.ageDistribution : [];
    ...
    <div>
      ...
      <div className="mt-10 grid grid-cols-1 gap-5 lg:grid-cols-2">
        <div>
          ...
          <BarChartCard data={topStations} xKey="name" yKey="count" layout="horizontal" color="#1E3A4F" highlightColor="#38BDF8" />
        </div>
        <div>
          ...
          <BarChartCard data={ageDistribution} xKey="age" yKey="percent" color="#1E3A4F" highlightColor="#38BDF8" />
        </div>
      </div>
      ...
      <div className="mt-10">
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {insights.map((insight, idx) => (
            <InsightCard key={insight.title || idx} {...insight} />
          ))}
        </div>
      </div>
