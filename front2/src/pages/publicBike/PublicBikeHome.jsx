...
export default function PublicBikeHome() {
  ...
  useEffect(() => {
    publicBikeService.getRoutes() 
      .then((res) => {
        const data = Array.isArray(res) ? res : (res?.data ?? []);
        const rawRoutes = Array.isArray(data) ? data : [];

        const enrichedRoutes = rawRoutes.map((route, index) => ({
          ...route,
          image: route.image || ROUTES_MOCK[index % ROUTES_MOCK.length]?.image || ROUTES_MOCK[0].image,
          region: route.region || "서울시",
          tags: route.tags || ["추천", "따릉이"]
        }));
