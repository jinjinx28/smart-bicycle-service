...
export default function DemandForecast() {
  ...
  // 1. 용량 한계를 정원수(rackCount) 그 자체로 설정 (정원수가 없으면 기본값 20)
  const capacityLimit = station?.rackCount ? station.rackCount : 20;

  // 2. 혼잡도 계산: (예측건수 / 정원수) * 100
  const barPct = (station && result?.predicted_demand != null) 
    ? Math.min(100, Math.round((result.predicted_demand / capacityLimit) * 100)) // 100으로 수정
    : 0;
