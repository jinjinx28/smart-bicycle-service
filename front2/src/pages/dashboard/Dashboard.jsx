...
export default function Dashboard() {
  ...
  const displayName = user?.username || user?.name || user?.email?.split("@")[0] || "게스트";

  const activityStats = [
    { icon: Trophy, label: "획득 뱃지", value: data?.activity?.badges ?? 0, tone: "text-warn" },
    { icon: Target, label: "달성 챌린지", value: data?.activity?.challenges ?? 0, tone: "text-neon" },
    { icon: Users, label: "팔로잉 라이더", value: data?.activity?.followers ?? 0, tone: "text-bike" },
    { icon: Star, label: "저장 루트", value: data?.activity?.savedRoutes ?? 0, tone: "text-warn" },
  ];
      ...
      <div className="mb-8 flex flex-wrap items-center justify-between gap-6 rounded-2xl border-l-4 border-neon bg-card p-8">
        <div>
          ...
          <div className="mt-4 flex items-center gap-2 text-sm text-gray-400">
            <Badge variant="neon">일반 라이더</Badge>
            <span>{user?.email || "@rider"}</span>
          </div>
          </div>
