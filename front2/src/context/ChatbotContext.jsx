...
export function ChatbotProvider({ children }) {
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    try {
      ...
      const botReply = typeof response === "string" ? response : (response?.reply || response?.data?.reply || "답변을 불러오지 못했습니다.");

      setMessages((prev) => [...prev, { id: `b-${Date.now()}`, role: "bot", text: botReply }]);
    } catch (e) {
      setMessages((prev) => [...prev, { id: `b-${Date.now()}`, role: "bot", text: "죄송합니다. 통신 중 문제가 발생했습니다." }]);
    } finally {
      setIsSending(false);
    }
  }, []);
